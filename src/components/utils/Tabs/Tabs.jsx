...
const Tabs = ({ children }) => (
  ...
  const TabsList = ({ children, variant }) => (
    <button className={`${
        variant === 'adminSlider' ? `bg-slate-700 p-1 mb-1 rounded-lg`:
        'bg-slate-500 p-1 mb-1 rounded-lg'
    }
    `}>
  ...
  const TabsContent = ({ children, variant }) => (
    <div className={` ${
        variant === 'adminSlider' ?  "px-4  py-3 bg-slate-700  rounded-lg text-slate-300":
        ''
    }`}
        >{children}</div>
  )
